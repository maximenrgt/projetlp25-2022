...
int main(int argc, char *argv[]) {
    configuration_t config = {
            .data_path = "", // maildir
            .temporary_directory = "", //dosier temporaire ou on stocke les fichiers
            .output_file = "", //fichier resultat final
            .is_verbose = false, // ??
            .cpu_core_multiplier = 2, //  ?? ne bouge jamais
    };
    ...
    config.process_count = get_nprocs() * config.cpu_core_multiplier; //compte nb coeurs
...
#ifdef METHOD_DIRECT
    direct_fork_directories(config.data_path /*maildir*/, config.temporary_directory, config.process_count); //1er mapper
    sync_temporary_files(config.temporary_directory);
    char direct_temp_result_name[STR_MAX_LEN];
    concat_path(config.temporary_directory, "step1_output", direct_temp_result_name); 
    files_list_reducer(config.data_path, config.temporary_directory, direct_temp_result_name); // 1er reducer
    direct_fork_files(config.data_path, config.temporary_directory, config.process_count); //2e mapper
    sync_temporary_files(config.temporary_directory);
    char direct_step2_file[STR_MAX_LEN];
    concat_path(config.temporary_directory, "step2_output", direct_step2_file);
    files_reducer(direct_step2_file, config.output_file); //dernier reducer
#endif
